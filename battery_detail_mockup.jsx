...
// ── Light theme design tokens (consistent with main console) ──
const C = {
  bg: '#f5f7fa',
  bgCard: '#ffffff',
  bgCardAlt: '#fafbfc',
...
const BatteryShape = ({ soc, charging = false }) => {
...
// ── Real-time data card ──
const DataMetric = ({ label, value, unit, color = C.text, big = false }) => (
  <div style={{
    background: C.bgCard,
...
export default function BatteryDetail() {
  ...
  const st = STATUS_MAP[BATTERY.status];
      ...
      <div style={{
        ...
        {['主控台', '电池管理', '充电站', '钻井队', '告警中心', '运营报表', '系统管理'].map((item, i) => (
          <span key={item} style={{
            ...
            color: i === 1 ? C.accent : C.textSec,
            padding: '8px 14px', borderRadius: 7,
            background: i === 1 ? C.accentLight : 'transparent',
          }}>{item}</span>
        ))}
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 20 }}>
            <span style={{ fontSize: 11, color: C.textMut }}>
              数据更新 <span style={{ color: C.accent, fontFamily: FONT_MONO, fontWeight: 700 }}>30s 前</span>
            </span>
          </div>
        </div>
      </div>
