...
// ── Light theme design tokens (consistent) ──
const C = {
  bg: '#f5f7fa',
  bgCard: '#ffffff',
  bgCardAlt: '#fafbfc',
...
const FormLabel = ({ children, required, hint }) => (
  <div style={{
...
const FormField = ({ label, required, hint, children, fullWidth }) => (
...
const TimeSlotSelector = ({ selected }) => (
...
// ── Main Form Page ──
export default function DrillingPlanForm() {
  return (
    <div style={{
      ...
      <div style={{
        ...
        {['主控台', '电池管理', '充电站', '钻井队', '告警中心', '运营报表', '系统管理'].map((item, i) => (
          <span key={item} style={{
            ...
            background: i === 3 ? C.accentLight : 'transparent',
          }}>{item}</span>
        ))}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginLeft: 8, paddingLeft: 16, borderLeft: `1px solid ${C.border}` }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <div>
              <div style={{ fontSize: 12, color: C.text, fontWeight: 600 }}>王调度员</div>
              <div style={{ fontSize: 10, color: C.textMut }}>客户调度员</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ padding: '24px 28px', maxWidth: 1480, margin: '0 auto' }}>
          ...
          <div style={{
            ...
            <div style={{ marginBottom: 28 }}>
              ...
              <FormField label="主要用电时段" hint="选择钻机高功率工作的时段（可多选）" required>
                <TimeSlotSelector selected={[2, 3, 4]} />
              </FormField>
            </div>
        ...
        <div style={{
          ...
          <span style={{ fontSize: 11, color: C.textMut }}>
            计划提交后，系统将在 <span style={{ color: C.accent, fontWeight: 700 }}>10 分钟内</span> 生成完整调度方案并通知您
          </span>
        </div>
      </div>
    </div>
