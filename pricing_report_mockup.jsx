...
const Stat = ({ label, value, unit, sub, color = C.text, size = 'md' }) => {
  const sizes = {
...
const DailySavingsChart = () => {
  ...
  const maxPct = Math.max(...DAILY_SAVINGS.map(d => d.pct));
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 20 }}>
            <span style={{ fontSize: 11, color: C.textMut }}>
              数据源 <span style={{ color: C.accent, fontFamily: FONT_MONO, fontWeight: 700 }}>BMS + 电价系统</span>
            </span>
            <span style={{ fontSize: 11, color: C.textMut }}>
              最后更新 <span style={{ color: C.text, fontFamily: FONT_MONO, fontWeight: 700 }}>2026-04-16 14:00</span>
            </span>
          </div>
        </div>
      </div>
